...
and the construction of an array of these pointers.
*/
...
int main() {
  ...
  YesNoFcn fcts[] = {intEqual, bothOdd, intGreater};

  for (int i = 0; i < 3; i++) {
    cout << "Calling findMatches() using function at fct[" << i
         << "] : " << endl;
    findMatches(arr1, arr2, 7, fcts[i]);
    cout << endl;
  }

  string names[] = {"intEqual", "bothOdd", "intGreater"};

  for (int i = 0; i < 3; i++) {
    cout << "Calling findMatches() using " << names[i] << " : " << endl;
    findMatches(arr1, arr2, 7, fcts[i]);
    cout << endl;
  }
