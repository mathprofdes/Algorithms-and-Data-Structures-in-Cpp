//============================================================================
// Name        : BinaryFiles007.cpp
// Author      : Don Spickler
// Version     : 4/18/2020
// Description : Binary File Example #7: Files of Structs.
//============================================================================
...
int main() {
	...
	while (getline(dataFile, line)) {
		...
		int lastpos = pos;
		pos = line.find('\t', pos + 1);
		LifeExpCountry.CountryName = trim_copy(
		...
		lastpos = pos;
		pos = line.find('\t', pos + 1);
		LifeExpCountry.IncomeGroup = trim_copy(
				line.substr(lastpos, pos - lastpos));
		...
		lastpos = pos;
		pos = line.find('\t', pos + 1);
		LifeExpCountry.BirthYear = atoi(
				trim_copy(line.substr(lastpos, pos - lastpos)).c_str());

		LifeExpCountry.LifeExpectancy = atof(
				trim_copy(line.substr(pos)).c_str());
	...
	LifeExpStore *LifeExpDataStore = new LifeExpStore[LifeExpData.size()];
	...
	for (unsigned int i = 0; i < LifeExpData.size(); i++) {
		...
		LifeExpDataStore[i] = data;
	}

	// Save to binary file.
	dataFile.open("LifeExpWorldFull.dat", ios::out | ios::binary);
	...
	dataFile.write(reinterpret_cast<char*>(LifeExpDataStore),
			sizeof(LifeExpStore) * LifeExpData.size());
	dataFile.close();
...
string trim_copy(string str) {
	return rtrim_copy(ltrim_copy(str));
}
